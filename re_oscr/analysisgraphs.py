...
class AnalysisGraphs():
    ...
    def create_overview_plots(
            self, compact_labels: bool = False,
        ...
        self.dps_bar_plot = LegendPlot(
            self._theme, y_font=y_font, viewport_background=viewport_background)
        self.dps_bar_plot.set_padding_fraction(0.01)
        self.dps_graph_plot = LegendPlot(
            self._theme, x_unit='s', viewport_background=viewport_background)
        self.dmg_bar_plot = LegendPlot(
            self._theme, x_unit='s', viewport_background=viewport_background)
    ...
    def plot_overview_data(
        ...
        """
        player_colours = dict(zip(dps_graph_data, self._theme['plot']['color_cycler']))
        self.plot_horizontal_bar(overview_table, self.dps_bar_plot, player_colours)
    ...
    def plot_horizontal_bar(
            self, table: list[list], plot_widget: LegendPlot,
            player_colours: dict[str, str] | None = None):
        """
        ...
        """
        sorted_table = sorted(table, key=lambda line: line[2], reverse=True)
        y_annotations = (
            tuple((index + 1, line[0] + line[1]) for index, line in enumerate(sorted_table)),)
        plot_widget.set_axis_ticks('left', y_annotations)
        x = tuple(line[2] for line in sorted_table)
        y = tuple(range(1, len(x) + 1))
        upper_bound = max(x) * 1.05 if max(x) > 0 else 1
        plot_widget.set_x_range(0, upper_bound, padding=0)
        plot_widget.clear()
        if self._theme['plot'].get('overview_bar_colours', False) and player_colours is not None:
            for y_position, magnitude, player in zip(y, x, sorted_table):
                color = player_colours.get(player[1], self._theme['defaults']['mfg'])
                bars = BarGraphItem(
                    x0=0, y=(y_position,), height=0.75, width=(magnitude,),
                    brush=color, pen=None)
                plot_widget.add_item(bars)
        else:
            bars = BarGraphItem(
                x0=0, y=y, height=0.75, width=x,
                brush=self._theme['defaults']['mfg'], pen=None)
            plot_widget.add_item(bars)
